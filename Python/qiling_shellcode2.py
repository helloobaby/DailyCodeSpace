...
# 将shellcode转为exe,正常执行的时候只会有3个dll,ntdll.dll kernelbase.dll kernel32.dll
...
def get1(ql:Qiling) -> None:
    ...
    print(Fore.GREEN+'len -> '+hex(len))
    ba = bytearray()
    
    for i in range(len):
        ba.extend(ql.mem.read(rsi,1))
    print(Fore.GREEN+str(ba))

def get2(ql:Qiling) -> None:
    rdx = ql.arch.regs.read('rdx')
    print(Fore.GREEN+hex(rdx))
...
# 64位的程序要x8664_windows,一开始各种模拟出错就是因为这个
ql = Qiling(code=shellcode, rootfs=r"rootfs/x8664_windows",archtype=QL_ARCH.X8664,ostype=QL_OS.WINDOWS,verbose=QL_VERBOSE.DISASM)

# setup

ql.hook_address(get1,0x140000030)

# 执行完mov     rdx, gs:[rdx+60h]
#ql.hook_address(get2,0x140000025+5)

ql.hook_address(stop,0x140000043+5)
