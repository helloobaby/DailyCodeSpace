...
class SocksProxy(StreamRequestHandler):
    ...
    def exchange_loop(self, client, remote):
        while True:
            ...
            # client套接字为本机代理服务器与代理程序之间
            if client in r:
                data = client.recv(4096)
                # 上游服务器ip+端口
                name,port = remote.getpeername()
                # 测试
                #if port == 8889:
                    #print(data)
            ...
            if remote in r:
                ...
                if client.send(data) <= 0:
                    # 可能是本机客户端关闭了,收不到转发的消息
                    break
if __name__ == '__main__':
    # 使用socketserver库的多线程服务器ThreadingTCPServer启动代理
    with ThreadingTCPServer(('127.0.0.1', 6666), SocksProxy) as server:
